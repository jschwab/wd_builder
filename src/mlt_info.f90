...
! ***********************************************************************
      ...
      !integer, parameter :: num_mlt_results = 13
...
!$OMP end critical (mlt_info_crit5)
         ...
         if (tau < 2d0/3d0) then ! B. Paczynski, 1969, Acta Astr., vol. 19, 1., eqn 14.
            s_gradr = (2d0*crad*T*T*T*sqrt(r))/(3d0*cgrav*m*rho)*pow_cr(L/(8d0*pi*boltz_sigma), 0.25d0) ! eqn 15
...
!$omp end critical (mlt_info_crit9)
         ...
         loglambdah = -19.26d0 - 0.5d0*log_cr(rho) + 1.5d0*log_cr(T) - 0.5d0*log_cr(1d0 + 0.5d0*(1+xh)) 
         nu_rad = 4d0*crad*T*T*T*T/(15d0*clight*opacity*rho*rho) ! radiative viscosity
         nu_mol = 0.406d0*sqrt(amu)*pow_cr(boltzm*T,2.5d0)/(qe4*loglambdah*rho) 
         ...
         Bcoeff = (15.d0/16.d0)*sqrt(2.d0*amu/(5*pi))*pow_cr(boltzm,2.5d0)/qe4
         ...
         ! This is if the driving chemical is NOT He.
            ...
            loglambdacy = loglambdah - log_cr(2.d0*chemz)
            ! Calculation of C_ij coeffs (equation 12)
            ccx = log_cr(exp_cr(1.2d0*loglambdacx)+1.)/1.2d0
            ccy = log_cr(exp_cr(1.2d0*loglambdacy)+1.)/1.2d0
            ...
            loglambdah = -19.26d0 - log_cr(2d0) - 0.5d0*log_cr(rho) + &
               1.5d0*log_cr(T) - 0.5d0*log_cr(1d0 + 0.5d0*(1+xh)) 
            ! Calculation of C_ij coeffs (equation 12)
            ccy = log_cr(exp_cr(1.2d0*loglambdah)+1d0)/1.2d0
            ! My formula (see notes) based on Proffitt and Michaud 1993
            kmu = (Bcoeff*pow_cr(T,2.5d0)/(rho*ccy))*(3+xh)/((1+xh)*(3+5*xh)*(0.7d0+0.3d0*xh))
...
!         write(*,*) a_2,a_1,a_0,b_2,b_1,b_0
         ...
         if(r_th .lt. 0.5d0) then
            if(r_th .gt. prandtl) then
               maxl = pow_cr((1.d0/(1.d0+phi)) - 2.d0*dsqrt(r_th*phi)/pow_cr(1d0+phi,2.5d0),0.25d0)   
                  ! Equation (B14)
